...
ScalarConverter::is_double (const std::string &parameter)
{
	...
	/// Step #2.2 Find decimal and floating parts
	std::string integral_part = parameter.substr (i, dot_index - i);
	std::string fractional_part = parameter.substr (
		dot_index + 1, parameter.length () - dot_index - 1);
	if (integral_part.empty () || fractional_part.empty ())
		return (false);
...
bool
ScalarConverter::is_pseudo_literal (const std::string &parameter)
{
	/// Checks for the valid pseudo literals
	return (parameter == "-inff" || parameter == "+inff" || parameter == "nanf"
			|| parameter == "-inf" || parameter == "+inf"
			|| parameter == "nan");
}
...
void
ScalarConverter::convert_data (const std::string &parameter,
							   long double number)
{
...
void
ScalarConverter::print_pseudoliterals (const std::string &parameter)
{
	...
	if (parameter.find ("nan") != std::string::npos)
		...
		{
			std::cout << "float: " << parameter[0] << "inff" << std::endl;
			std::cout << "double: " << parameter[0] << "inf" << std::endl;
		}
}
